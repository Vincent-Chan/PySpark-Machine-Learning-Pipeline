...
Original file is located at
    https://colab.research.google.com/drive/1ASZDxDmTsOqJy2grjWriZvPJH8UbX1Li
...
# correct answer should be below:
# sqlContext.sql("select PE as PowerOutput, AT as AtmosphericTemperature from plant_dataset").toPandas()
...
sqlContext.sql("select V as VacuumSpeed, PE as PowerOutput from plant_dataset").toPandas()
...
sqlContext.sql("select AP as AtmosphericPressure, PE as PowerOutput from plant_dataset").toPandas()
...
sqlContext.sql("select RH as RelativeHumidity, PE as PowerOutput from plant_dataset").toPandas()
...
rf.setPredictionCol("Prediction_PE").setLabelCol("PE").setFeaturesCol("features").setSeed(190088121).setMaxDepth(8).setNumTrees(25)
